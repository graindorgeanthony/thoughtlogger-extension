...
export async function connect(storage = chrome.storage.local, identity = chrome.identity, fetchImpl = fetch) {
  ...
  const callbackUrl = await identity.launchWebAuthFlow({ url: authorize.href, interactive: true });
